...
export function HSKLevelCard({ meta, knownCount }: { meta: LevelMeta; knownCount: number }) {
  ...
  const tones = {
    ...
    green: "from-emerald-400 via-mint to-lime-100",
  ...
  return (
    <div className={`group relative min-h-[360px] overflow-hidden rounded-[2.35rem] bg-gradient-to-br ${tones[meta.accent]} p-6 shadow-premium transition duration-300 hover:-translate-y-2 hover:shadow-glow ${meta.locked ? "grayscale-[0.25]" : ""}`}>
      ...
      {meta.locked ? <div className="absolute inset-0 bg-white/30 backdrop-blur-[1px] dark:bg-black/20" /> : null}
      <div className="relative z-10">
        ...
        <div className="mt-7 rounded-[1.75rem] bg-white/70 p-4 shadow-soft backdrop-blur dark:bg-white/12">
          <div className="flex items-center gap-3">
            <ShieldCheck className="h-5 w-5 text-orange-deep" />
            <div>
              <h2 className="text-2xl font-black text-ink dark:text-cream">{meta.wordCount} {t("common.words")}</h2>
              <p className="text-sm font-bold text-stone-600 dark:text-stone-300">{t("roadmap.unlockRule")}</p>
            </div>
          </div>
        </div>
        <div className="mt-6">
          ...
          <div className="h-3 rounded-full bg-white/55">
            <div className="h-3 rounded-full bg-ink shadow-soft" style={{ width: `${progress}%` }} />
          </div>
        </div>
        <div className="mt-6">
