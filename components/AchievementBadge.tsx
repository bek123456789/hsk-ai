...
export function AchievementBadge({ icon: Icon, title, detail, unlocked = true }: { icon: LucideIcon; title: string; detail: string; unlocked?: boolean }) {
  return (
    <div className={`rounded-[1.75rem] border p-4 shadow-soft transition hover:-translate-y-1 ${
      unlocked
        ? "border-white/70 bg-white/82 dark:border-white/10 dark:bg-white/10"
        : "border-white/60 bg-white/45 opacity-60 dark:border-white/10 dark:bg-white/5"
    }`}>
      ...
      <p className="font-black text-ink dark:text-cream">{title}</p>
      <p className="mt-1 text-xs font-bold leading-5 text-stone-500 dark:text-stone-300">{detail}</p>
    </div>
  );
}
