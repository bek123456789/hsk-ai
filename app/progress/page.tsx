...
export default function ProgressPage() {
  ...
  return (
    ...
    <section className="premium-grid mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:py-14">
      <div className="mb-8">
        <p className="text-sm font-black uppercase tracking-normal text-orange-deep dark:text-orange-200">{t("progress.title")}</p>
        <h1 className="mt-2 text-5xl font-black text-ink dark:text-cream sm:text-7xl">{t("progress.dashboard")}</h1>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-5">
        ...
        <ProgressCard title={t("common.dailyStreak")} value={`${streak}`} detail={t("progress.localStreak")} icon={Flame} tone="green" />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-5xl border border-white/70 bg-white/82 p-7 shadow-premium backdrop-blur-xl dark:border-white/10 dark:bg-white/10">
          <h2 className="text-2xl font-black text-ink dark:text-cream">{t("progress.hskProgress")}</h2>
          <div className="mt-6 rounded-4xl bg-cream p-5 dark:bg-obsidian/60">
            <div className="mb-2 flex justify-between text-sm font-black text-stone-600">
              <span>HSK 1</span>
              <span>{hskProgress}%</span>
            </div>
            <div className="h-4 rounded-full bg-white dark:bg-white/10">
          ...
          <div className="mt-5 space-y-3">
            {quizResults.slice(0, 3).map((result) => (
              <div key={result.completedAt} className="flex items-center justify-between rounded-3xl bg-orange-soft px-4 py-3 shadow-soft dark:bg-orange-brand/18">
                <span className="font-black text-ink dark:text-cream">HSK {result.level} {t("common.quiz")}</span>
                <span className="font-black text-orange-deep">{result.score}/{result.total}</span>
              </div>
            ))}
            {!quizResults.length ? <p className="text-sm font-semibold text-stone-500 dark:text-stone-300">{t("progress.completeQuiz")}</p> : null}
          </div>
        ...
        <div className="rounded-5xl border border-white/70 bg-white/82 p-7 shadow-premium backdrop-blur-xl dark:border-white/10 dark:bg-white/10">
          <h2 className="text-2xl font-black text-ink dark:text-cream">{t("progress.recentWords")}</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {recentWords.length ? recentWords.map((word) => (
              <div key={word.id} className="rounded-4xl bg-cream p-5 shadow-soft dark:bg-obsidian/60">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-4xl font-black text-ink dark:text-cream">{word.chinese}</p>
                    <p className="mt-1 font-black text-orange-brand">{word.pinyin}</p>
                  </div>
                  <p className="text-right text-sm font-black text-stone-600 dark:text-stone-300">{word.translationUz}</p>
                </div>
              </div>
            )) : (
              <p className="text-sm font-semibold text-stone-500 dark:text-stone-300">{t("progress.emptyRecent")}</p>
            )}
          </div>
        </div>
      </div>
