...
import { ArrowLeft, ArrowRight, Check, PartyPopper, Sparkles, X } from "lucide-react";
    ...
    <section className="premium-grid relative mx-auto max-w-5xl overflow-hidden px-5 pb-36 pt-10 sm:px-8 md:pb-10 lg:py-14">
      ...
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <div className="min-w-[160px] text-right">
          <p className="text-sm font-black text-stone-500 dark:text-stone-300">{Math.min(index + 1, words.length)} / {words.length}</p>
      ...
      {done ? (
        <div className="relative overflow-hidden rounded-[2.5rem] border border-white/70 bg-white/82 p-10 text-center shadow-premium backdrop-blur-xl dark:border-white/10 dark:bg-white/10">
          ...
          <h1 className="text-5xl font-black text-ink dark:text-cream">{t("flashcard.complete")}</h1>
          <p className="mt-3 font-semibold text-stone-500 dark:text-stone-300">{t("flashcard.saved")}</p>
        ...
        <>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <button onClick={() => next(false)} className="inline-flex items-center justify-center gap-2 rounded-full bg-white/82 px-6 py-4 text-sm font-black text-ink shadow-soft backdrop-blur transition hover:-translate-y-1 dark:bg-white/10 dark:text-cream">
          ...
          <div className="mt-5 text-center">
            <button onClick={() => next(false)} className="inline-flex items-center gap-2 text-sm font-black text-stone-500 hover:text-orange-deep dark:text-stone-300">
