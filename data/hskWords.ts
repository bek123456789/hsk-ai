...
const hsk1BaseWords = [
  ...
  { id: "hsk1-011", hskLevel: 1, lessonId: "hsk1-people", chinese: "他们", pinyin: "tā men", translationUz: "ular", exampleChinese: "他们学习汉语。", exampleUz: "Ular xitoy tilini o‘rganadi." },
...
const generatedWords: HSKWord[] = hskVocabulary
  .filter((word) => word.hskLevel !== 1)
  .map((word) => ({
    id: word.id,
    ...
    translationZh: word.translationRu,
    exampleChinese: word.exampleChinese,
    exampleUz: word.exampleUz,
    exampleRu: word.exampleRu,
    exampleZh: word.exampleRu
  }));

const legacyWords = [...hsk1Words, ...generatedWords];
const legacyWordKeys = new Set(legacyWords.map((word) => `${word.hskLevel}:${word.chinese}`));

const expandedWords: HSKWord[] = vocabularyEntries
  .filter((word) => !legacyWordKeys.has(`${word.level}:${word.hanzi}`))
  .map((word) => ({
    id: word.id,
    hskLevel: word.level,
    lessonId: `hsk${word.level}-${word.category}`,
    chinese: word.hanzi,
    pinyin: word.pinyin,
    translationUz: word.uz,
    translationRu: word.ru,
    translationZh: word.ru,
...
export const hskWords: HSKWord[] = [...legacyWords, ...expandedWords];

const generatedLessons: HSKLesson[] = hskCourseLessons
  .filter((lesson) => lesson.hskLevel !== 1)
  .map((lesson) => ({
    id: lesson.lessonId,
    hskLevel: lesson.hskLevel,
    title: lesson.titleUz,
    titleRu: lesson.titleRu,
    description: lesson.shortDescriptionUz,
    descriptionRu: lesson.shortDescriptionRu,
    wordIds: lesson.vocabulary.map((word) => word.id),
    locked: false
  }));
...
export const hskLessons: HSKLesson[] = [
  {
    ...
    descriptionRu: "Начните разговор с простых вежливых фраз.",
  ...
  },
  {
    ...
    descriptionRu: "Говорите о себе, друзьях, учителях и Китае.",
  ...
  },
  {
    ...
    titleRu: "Урок 3: Вопросы",
    description: "Kim, nima, qayer, qancha va qaysi savollarini ishlating.",
    descriptionRu: "Практикуйте вопросы кто, что, где, сколько и какой.",
  ...
  },
  {
    ...
    titleRu: "Урок 4: Ежедневные действия",
    description: "O‘qish, yeyish, borish, kelish va ko‘rish fe’llarini mashq qiling.",
    descriptionRu: "Практикуйте глаголы учиться, есть, идти, приходить и смотреть.",
...
export const levelMeta: LevelMeta[] = [
  ...
  { level: 2, wordCount: 300, locked: true, accent: "blue" },
